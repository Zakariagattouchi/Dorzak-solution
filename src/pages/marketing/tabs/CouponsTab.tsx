...
const couponStatus = (c: CouponRow): { status: string; label: string } => {
  ...
  if (c.expires_at && new Date(c.expires_at) < new Date())
    return { status: 'CANCELLED', label: 'Expired' };
  if (c.usage_limit != null && c.used_count >= c.usage_limit)
    return { status: 'CANCELLED', label: 'Exhausted' };
...
export const CouponsTab: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  const load = useCallback(async () => {
    try {
      setCoupons(((await couponsApi.list()) as any).coupons ?? []);
    } catch {
      /* locked or empty */
    }
    setLoading(false);
  ...
  const create = async () => {
    if (!form.code.trim() || !form.value) {
      addToast('A code and a value are required.', 'warning');
      return;
    }
    setSaving(true);
    try {
  ...
  const remove = async (c: CouponRow) => {
    try {
      ...
      addToast(`Coupon ${c.code} deleted.`, 'success');
      await load();
    } catch (e) {
      addToast(apiErrorMessage(e), 'danger');
    }
  ...
  const columns: Column<CouponRow>[] = [
    ...
    },
    {
      ...
      render: (c) => (
        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          {[
            c.min_order > 0 ? `min ${money(c.min_order)}` : null,
            c.expires_at ? `until ${c.expires_at}` : null,
          ]
            .filter(Boolean)
            .join(' · ') || '—'}
        </span>
    ...
    },
    {
      key: 'usage',
      header: 'Used',
      render: (c) => `${c.used_count}${c.usage_limit != null ? ` / ${c.usage_limit}` : ''}`,
    },
    ...
    { key: 'revenue', header: 'Revenue driven', render: (c) => <b>{money(c.revenue)}</b> },
    {
      ...
      render: (c) => (
        <span style={{ color: 'var(--dorzak-danger)' }}>-{money(c.discount_given)}</span>
      ),
    },
    {
      key: 'status',
      ...
      ? [
          {
            ...
            render: (c: CouponRow) => (
              ...
              >
                <AppButton variant="tertiary" onClick={() => toggle(c)}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <TabIntro
        title="Coupon codes"
        sub="Discount codes customers apply at checkout — tracked down to the revenue each code drives."
      >
        {isAdmin && (
      ...
      {loading ? (
        ...
        >
          No coupons yet. Create your first code — customers enter it at checkout and you'll see the
          orders it drives here.
        </div>
      ) : (
        <DataTable
          columns={columns}
