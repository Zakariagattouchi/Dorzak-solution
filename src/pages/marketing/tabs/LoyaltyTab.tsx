...
export const LoyaltyTab: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  const liability =
    (stats.points_outstanding / Math.max(1, Number(redeemPoints) || 1)) *
    (Number(redeemValue) || 0);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <TabIntro
        title="Loyalty program"
        sub="Customers earn points on every completed order and spend them as a discount at checkout."
      />
      ...
      >
        ...
        <StatCard
          ...
          tone="muted"
          hint="if every point were spent today"
        />
      </div>
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          Example with these settings: a customer spending {money(100)} earns{' '}
          {(Number(earnRate) || 0) * 100} points; {redeemPoints} points take{' '}
          {money(Number(redeemValue) || 0)} off a future order.
        </p>
        {isAdmin && (
          <div>
