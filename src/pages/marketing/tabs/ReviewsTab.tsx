...
const Stars: React.FC<{ rating: number }> = ({ rating }) => (
  <span
    ...
    style={{ color: 'var(--dorzak-warning, #d97706)', letterSpacing: 1 }}
  >
    {'★'.repeat(rating)}
...
export const ReviewsTab: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  const load = useCallback(async () => {
    try {
      setReviews(((await reviewsApi.list()) as any).reviews ?? []);
    } catch {
      /* locked */
    }
    setLoading(false);
  ...
  const columns: Column<ReviewRow>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <StatusPill
          status={r.approved ? 'ACTIVE' : 'CONFIRMING'}
          label={r.approved ? 'Published' : 'Pending'}
        />
      ),
    },
    ...(isAdmin
      ? [
          {
            ...
            render: (r: ReviewRow) => (
              ...
              >
                {!r.approved && (
                  <AppButton variant="secondary" onClick={() => approve(r)}>
                    Publish
                  </AppButton>
                )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <TabIntro
        ...
        sub="Reviews wait here until you publish them — nothing appears on your storefront unmoderated."
      >
        <div style={{ display: 'flex', gap: 6 }}>
          <AppButton
            variant={filter === 'PENDING' ? 'primary' : 'tertiary'}
      ...
      >
        <StatCard
          ...
          value={approved.length ? average.toFixed(1) : '—'}
          tone="success"
          hint={`${approved.length} published`}
        />
        <StatCard
          label="Awaiting moderation"
          value={pending.length}
          tone={pending.length ? 'danger' : 'muted'}
        />
      </div>
      ...
      {loading ? (
        ...
        >
          {filter === 'PENDING'
            ? 'No reviews waiting — all caught up.'
            : 'No reviews yet. They arrive here when customers rate products.'}
        </div>
      ) : (
        <DataTable
          columns={columns}
