...
export const ReferralsTab: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await referralApi.update({
        enabled,
        referrer_reward: Number(referrerReward) || 0,
        referee_reward: Number(refereeReward) || 0,
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <TabIntro
        ...
        sub="Customers share a personal code; when a friend places their first order, both earn store credit."
      />
      ...
      >
        <StatCard
          ...
          tone="success"
          hint="first orders completed"
        />
        <StatCard
          label="Pending"
          value={stats.pending}
          tone="muted"
          hint="referred, not yet ordered"
        />
        <StatCard
          label="Credit issued"
          value={money(stats.credit_issued)}
          hint="total rewards paid out"
        />
      </div>

      <div
        className="card"
        style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 560 }}
      >
        <ToggleSwitch
          checked={enabled}
          onChange={setEnabled}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          />
          <TextInput
            label="Welcome reward for the friend"
            type="number"
            min="0"
            value={refereeReward}
        ...
        <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)' }}>
          Rewards are paid as store credit the moment the friend's first order completes.
          Self-referrals and repeat referrals are blocked automatically.
        </p>
        {isAdmin && (
          <div>
