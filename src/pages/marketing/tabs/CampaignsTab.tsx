...
export const CampaignsTab: React.FC<{
  ...
  const load = useCallback(async () => {
    try {
      setCampaigns(((await campaignsApi.list()) as any).campaigns ?? []);
    } catch {
      /* locked */
    }
    try {
      setChannels(((await messagingApi.get()) as any).status ?? null);
    } catch {
      /* not fatal */
    }
    if (hasSegments) {
  ...
  useEffect(() => {
    if (channels && !form.channel) {
      setForm((f) => ({
        ...f,
        channel: channels.email.ready ? 'email' : channels.whatsapp.ready ? 'whatsapp' : '',
      }));
    }
  ...
  const submit = async (sendNow: boolean) => {
    if (!form.channel) {
      addToast('Set up a channel first.', 'warning');
      return;
    }
    ...
    if (form.audience_type === 'segment' && !form.segment_id) {
      addToast('Pick a segment for this audience.', 'warning');
      return;
    }
    setSaving(true);
    try {
      const created = (await campaignsApi.create({
        ...
        wa_template_language:
          form.channel === 'whatsapp' ? form.wa_template_language || 'en' : null,
        audience:
          form.audience_type === 'segment'
            ? { type: 'segment', segment_id: Number(form.segment_id) }
            : { type: 'all' },
      ...
      if (sendNow) {
        ...
        addToast(
          `Campaign sent to ${res.sent_count} customer${res.sent_count === 1 ? '' : 's'}.`,
          'success',
        );
      } else {
  ...
  const sendNow = async (c: CampaignRow) => {
    try {
      ...
      addToast(
        `"${c.subject}" sent to ${res.sent_count} customer${res.sent_count === 1 ? '' : 's'}.`,
        'success',
      );
      await load();
    } catch (e) {
  ...
  const audienceLabel = (c: CampaignRow) =>
  ...
  const channelOptions = channels
    ? [
        {
          value: 'email',
          label: channels.email.ready ? 'Email' : 'Email — not set up',
          disabled: !channels.email.ready,
        },
        {
          value: 'whatsapp',
          label: channels.whatsapp.ready
            ? `WhatsApp${channels.whatsapp.display_number ? ` (${channels.whatsapp.display_number})` : ''}`
            : 'WhatsApp — not connected',
          disabled: !channels.whatsapp.ready,
        },
      ]
    : [];
  ...
  const columns: Column<CampaignRow>[] = [
    ...
    { key: 'audience', header: 'Audience', render: (c) => audienceLabel(c) },
    {
      ...
      render: (c) => (
        <StatusPill
          status={
            c.status === 'sent' ? 'COMPLETE' : c.status === 'scheduled' ? 'CONFIRMING' : 'PENDING'
          }
      ...
      render: (c) =>
        c.status === 'sent' ? (
          <span>
            {c.sent_count}
            {c.failed_count > 0 && (
              <span style={{ color: 'var(--dorzak-danger)', fontSize: '0.8rem' }}>
                {' '}
      ...
      ? [
          {
            ...
            render: (c: CampaignRow) => (
              ...
              >
                {c.status !== 'sent' && (
                  <AppButton variant="secondary" onClick={() => sendNow(c)}>
                    Send now
                  </AppButton>
                )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <TabIntro
        title="Campaigns"
        sub="Reach your customers by email or WhatsApp — everyone, or a saved segment."
      >
        {isAdmin && anyChannelReady && (
      ...
      {channels && !anyChannelReady && (
        ...
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '0.9rem' }}>
            <AppIcon name="alert" size={16} /> No sending channel is set up yet — campaigns can't go
            out until email or WhatsApp is configured.
          </span>
          {isAdmin && (
            <AppButton variant="primary" onClick={onOpenChannels}>
              Set up channels
            </AppButton>
          )}
        </div>
      ...
      {showForm && (
        ...
        >
          <TextInput
            ...
            placeholder="Weekend sale — 20% off everything"
          />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
                ...
                <TextInput
                  ...
                  onChange={(e) => setForm({ ...form, wa_template_language: e.target.value })}
                  placeholder="en"
                />
              </div>
              <p
              ...
              >
                <AppIcon name="info" size={14} /> Meta requires an approved template for marketing
                messages outside a 24-hour customer session. Leave blank only if you're replying
                within active sessions — the message text above is used instead.
              </p>
            </div>
          )}
          <div style={{ display: 'flex', gap: 8 }}>
            <AppButton variant="primary" loading={saving} onClick={() => submit(true)}>
              Send now
            </AppButton>
            <AppButton variant="secondary" loading={saving} onClick={() => submit(false)}>
      ...
      {loading ? (
        ...
        >
          No campaigns yet. Announce a sale, a new product, or a slow-day offer — it reaches every
          customer with contact details on file.
        </div>
      ) : (
        <DataTable
          columns={columns}
