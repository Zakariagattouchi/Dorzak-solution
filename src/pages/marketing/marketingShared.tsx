...
export const TabIntro: React.FC<{ title: string; sub: string; children?: React.ReactNode }> = ({
  title,
  sub,
  children,
}) => (
...
export const ConfirmButton: React.FC<{
  ...
  confirmLabel?: string;
  onConfirm: () => void;
}> = ({ label = 'Delete', confirmLabel = 'Confirm?', onConfirm }) => {
...
export const LockedFeature: React.FC<{ title: string; description: string }> = ({
  title,
  description,
}) => {
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: 0, maxWidth: 420, color: 'var(--text-muted)', fontSize: '0.9rem' }}>
        {description}
      </p>
