...
export const SalesPage: React.FC = () => {
  ...
  const columns: Column<Order>[] = [
    ...
    { key: 'date', header: 'Date & Time' },
    {
      ...
      render: (row) => {
        ...
        const colors: Record<string, string> = {
          CARD: 'var(--dorzak-primary)',
          CASH: 'var(--dorzak-success)',
          TRANSFER: 'var(--color-info)',
          WHATSAPP: 'var(--dorzak-success)',
        };
        return (
          <span
            style={{
              ...
              fontSize: '0.8rem',
              fontWeight: 700,
              backgroundColor: `${colors[row.paymentMethod]}1a`,
              color: colors[row.paymentMethod],
            }}
          >
    ...
    },
    {
      ...
      render: (row) =>
        row.discount > 0 ? (
          <span style={{ color: 'var(--dorzak-danger)', fontWeight: 600 }}>
            -{money(row.discount)}
          </span>
        ) : (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
        <div className="card" style={{ padding: '14px' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--dorzak-success)' }}>
            {money(totalRevenue)}
          </div>
        </div>
        <div className="card" style={{ padding: '14px' }}>
        ...
        <div className="card" style={{ padding: '14px' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--dorzak-danger)' }}>
            -{money(totalDiscount)}
          </div>
        </div>
      </div>
          ...
          {(['ALL', 'CARD', 'CASH', 'TRANSFER', 'WHATSAPP'] as PayFilter[]).map((pf) => (
            <button
              ...
              style={{
                ...
                backgroundColor:
                  payFilter === pf ? 'var(--dorzak-primary)' : 'var(--color-surface)',
