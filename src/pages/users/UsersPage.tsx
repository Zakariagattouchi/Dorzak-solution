  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '900px' }}>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <strong style={{ color: 'var(--dorzak-primary)', fontSize: '0.9rem' }}>
            Dorzak Merchant Pro Plan
          </strong>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ padding: '12px', backgroundColor: 'var(--color-bg)', borderRadius: '8px' }}>
            <strong style={{ fontSize: '0.85rem' }}>
              {ROLE_CONFIG[inviteRole].label} Permissions:
            </strong>
            <ul style={{ margin: '6px 0 0 0', paddingLeft: '16px' }}>
              {ROLE_CONFIG[inviteRole].permissions.map((p) => (
                <li
                  key={p}
                  style={{ fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: '3px' }}
                >
                  {p}
                </li>
              ))}
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <AppButton variant="secondary" onClick={() => setShowInviteForm(false)}>
              Cancel
            </AppButton>
          ...
          >
            ...
            <div style={{ marginTop: '4px' }}>
              {cfg.permissions.slice(0, 2).map((p) => (
                <div key={p} style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                  • {p}
                </div>
              ))}
            </div>
