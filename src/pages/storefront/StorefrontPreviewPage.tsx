...
const labels = {
  en: {
    ...
    where: 'Where should we deliver?',
    pinHelp: 'Search or tap the map to pin your exact location.',
    addressFound: 'Address',
    addressDetails: 'Building, floor, apartment or nearby landmark',
    contact: 'Who should we contact?',
    contactHelp: 'We only use this to coordinate your order.',
    paymentHelp: 'Choose how you want to complete payment.',
    ...
    tableHelp: 'This order will be sent to the staff with your table number.',
    chooseTable: 'Choose your table',
    tableLocked: 'You scanned this table QR code.',
    whatsapp: 'WhatsApp',
    fawran: 'Fawran Transfer',
    cashAtTable: 'Pay at table',
    cashAtTableHelp: 'A staff member will collect payment at your table.',
    fawranDescription:
      'Transfer the total to the Fawran account below. Your name is the transfer reference. Upload your receipt to complete the order.',
    fawranAlias: 'Alias',
    fawranIban: 'IBAN',
    fawranReferenceNote: 'Reference: your name (automatically inserted)',
    reference: 'Reference',
    shareOrder: 'Share order on WhatsApp',
    shareOrderHelp: 'Send your order details to the restaurant via WhatsApp.',
    trackOrder: 'Track your order',
    trackOrderHelp: 'You will be redirected automatically in {{seconds}} seconds.',
    shareThenTrack: 'Share on WhatsApp, then track your order below.',
    proof: 'Payment Receipt',
    proofRequired: 'Upload your transfer receipt to place the order.',
    place: 'Place Order',
    confirmation: 'Order confirmed',
    location: 'Delivery Location',
    options: 'Choose options',
    continue: 'Add selected item',
    empty: 'Your bag is empty',
    back: 'Continue shopping',
    customerInfo: 'Your information',
    customerInfoHelp: 'Enter your phone number to auto-fill your details.',
    subtotal: 'Subtotal',
    ...
    deliveryPendingConfirm:
      'The delivery fee will be added once the store confirms it — track your order below.',
  ...
  ar: {
    ...
    total: 'الإجمالي',
    close: 'إغلاق',
    fulfillment: 'كيف تود استلام طلبك؟',
    where: 'أين تريد توصيل الطلب؟',
    pinHelp: 'ابحث أو اضغط على الخريطة لتحديد موقعك بدقة.',
    addressFound: 'العنوان',
    addressDetails: 'المبنى، الطابق، الشقة أو أقرب معلم',
    contact: 'كيف نتواصل معك؟',
    contactHelp: 'نستخدم هذه البيانات فقط لتنسيق طلبك.',
    paymentHelp: 'اختر طريقة إتمام الدفع.',
    details: 'بياناتك',
    name: 'الاسم الكامل',
    phone: 'رقم الهاتف',
    ...
    tableHelp: 'سيصل الطلب للموظفين مع رقم طاولتك.',
    chooseTable: 'اختر طاولتك',
    tableLocked: 'تم فتح الطلب من رمز هذه الطاولة.',
    whatsapp: 'واتساب',
    fawran: 'تحويل فوراً',
    cashAtTable: 'الدفع على الطاولة',
    cashAtTableHelp: 'سيقوم الموظف بتحصيل الدفع على طاولتك.',
    fawranDescription:
      'يرجى تحويل المبلغ الإجمالي إلى حساب فوران أدناه. اسمك هو مرجع التحويل. ارفع إيصال الدفع لإتمام الطلب.',
    fawranAlias: 'الاسم المستعار',
    fawranIban: 'رقم الآيبان',
    fawranReferenceNote: 'المرجع: اسمك (يُدرج تلقائياً)',
    reference: 'المرجع',
    shareOrder: 'مشاركة الطلب على واتساب',
    shareOrderHelp: 'أرسل تفاصيل طلبك إلى المطعم عبر واتساب.',
    trackOrder: 'تتبع طلبك',
    trackOrderHelp: 'سيتم تحويلك تلقائياً خلال {{seconds}} ثانية.',
    shareThenTrack: 'شارك عبر واتساب، ثم تتبع طلبك أدناه.',
    proof: 'إيصال الدفع',
    proofRequired: 'يرجى رفع إيصال التحويل لإتمام الطلب.',
    place: 'تأكيد الطلب',
    confirmation: 'تم تأكيد الطلب',
    location: 'موقع التوصيل',
    options: 'اختر المواصفات',
    continue: 'إضافة المنتج المحدد',
    empty: 'السلة فارغة',
    back: 'متابعة التسوق',
    customerInfo: 'معلوماتك',
    customerInfoHelp: 'أدخل رقم هاتفك لملء بياناتك تلقائياً.',
...
export const StorefrontPreviewPage: React.FC = () => {
  ...
  type DeliveryOption = {
    ...
    fee: number;
    distance_km: number;
    is_plan_perk: boolean;
  };
  const [quote, setQuote] = useState<{
    status: 'idle' | 'loading' | 'quoted' | 'pending' | 'unavailable';
    fee?: number;
  ...
  useEffect(() => {
    if (!slug) {
      ...
      setStore({
        ...
        currency_symbol: localSettings.currencySymbol,
        symbol_placement: localSettings.symbolPlacement,
        bio: localSettings.storeBio,
        banner_url: localSettings.bannerUrl,
        logo_url: localSettings.logoUrl,
        accent_color: localSettings.storeAccentColor,
        allow_delivery: localSettings.allowDelivery,
        allow_pickup: localSettings.allowPickup,
        allow_dine_in: (localSettings as any).allowDineIn,
        ...
        fawran_enabled: localSettings.fawranEnabled,
        fawran_alias: localSettings.fawranAlias,
        fawran_mobile: localSettings.fawranMobile,
        fawran_iban: localSettings.fawranIban,
    ...
    Promise.all([publicApi.store(slug), publicApi.catalog(slug)]).then(
      ([storeResponse, catalogResponse]: any[]) => {
  ...
  useEffect(() => {
    ...
    if (fulfillment === 'delivery' && !store.allow_delivery) {
      ...
      setFulfillment(
        store.allow_delivery ? 'delivery' : store.allow_dine_in ? 'dine_in' : 'pickup',
      );
  ...
  const money = (value: number) => {
    const symbol =
      store?.currency === 'QAR' ? 'QAR' : (store?.currency_symbol ?? store?.currency ?? '');
    const amount = value.toFixed(2);
    return store?.symbol_placement === 'AFTER' ? `${amount} ${symbol}` : `${symbol} ${amount}`;
  };

  const total = bag.reduce(
    (sum, line) => sum + (line.variant?.price ?? line.product.price) * line.quantity,
    0,
  );
    ...
    const timer = setTimeout(async () => {
      try {
        const res: any = await publicApi.deliveryQuote(effectiveSlug, {
          ...
          lng: String(customer.longitude),
          subtotal: String(total),
          ...(deliveryProviderId != null
            ? { delivery_provider_id: String(deliveryProviderId) }
            : {}),
        });
        const d = res.data;
  ...
  const tableValid =
    fulfillment !== 'dine_in' ||
    (tableNumber != null && tableNumber > 0 && (tableCount === 0 || tableNumber <= tableCount));
  const canSubmit =
    !!bag.length &&
    !!customer.name &&
    !!customer.phone &&
  ...
  const displayTotal =
    fulfillment === 'delivery' && quote.status === 'quoted' ? total + (quote.fee ?? 0) : total;
  ...
  const selectedVariant = useMemo(
    () =>
      selecting?.variants.find(
        (variant) =>
          variant.isActive &&
          Object.entries(selectedOptions).every(
            ([groupId, optionId]) => variant.optionValues[groupId] === optionId,
          ) &&
          Object.entries(variant.optionValues).every(
            ([groupId, optionId]) => selectedOptions[groupId] === optionId,
          ),
      ),
  ...
  const addLine = (product: Product, variant?: ProductVariant) => {
    ...
    setBag((current) => {
      const exists = current.find(
        (line) => `${line.product.id}:${line.variant?.id ?? 'base'}` === key,
      );
      return exists
        ? current.map((line) =>
            `${line.product.id}:${line.variant?.id ?? 'base'}` === key
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      if (customer.longitude != null)
        form.append('customer[longitude]', String(customer.longitude));
      form.append('fulfillment', fulfillment);
      if (fulfillment === 'delivery' && deliveryProviderId != null)
        form.append('delivery_provider_id', String(deliveryProviderId));
      if (fulfillment === 'dine_in' && tableNumber != null)
        form.append('table_number', String(tableNumber));
      ...
      <header ref={headerRef} className="mobile-shopbar">
        ...
        <div className="shopbar-actions">
          {!checkout && !confirmation && pendingOrder && (
            <button
              className={`status-toggle-button ${statusChangedAt != null ? 'status-changed' : ''}`}
              onClick={() =>
                navigate(`/store/${pendingOrder.slug}/orders/${pendingOrder.orderNumber}`)
              }
          ...
          <button
            ref={cartButtonRef}
            className={`bag-icon-button ${itemCount === 0 ? 'bag-empty' : ''}`}
            onClick={() => itemCount > 0 && setCheckout(true)}
            disabled={itemCount === 0}
            aria-label={`${t.bag}, ${itemCount} ${t.products}`}
          >
            <AppIcon name="cart" />
          ...
          {store.show_store_header && (
            <div>
              <span className="hero-eyebrow">
                {store.allow_dine_in ? t.dineIn : store.allow_delivery ? t.delivery : t.pickup}
              </span>
              <h2
                style={
                  !store.show_store_gradient && !store.banner_url ? { color: '#17201e' } : undefined
                }
              >
                {store.bio || t.collection}
              </h2>
            </div>
          )}
        </section>
      ...
      <main className="store-content">
        {!checkout && !confirmation && (
          <>
            <section className="catalog-toolbar">
              <label className="store-search">
                ...
                <input
                  ...
                  aria-label={t.search}
                />
                <span className="store-search-count">
            ...
            <div className="store-categories" aria-label="Product categories">
              ...
              >
                <span className="category-thumb category-thumb-all">
              ...
              {categories.map((category) => (
                ...
                >
                  {category.imageUrl && <img src={category.imageUrl} loading="lazy" alt="" />}
            ...
            {groupedProducts.map(([categoryName, categoryProducts]) => (
              <section className="store-category-section" key={categoryName}>
                ...
                <div
                  className={`store-grid ${store.product_card_layout === 'horizontal' ? 'store-grid-horizontal' : ''}`}
                >
                    ...
                    <article className="store-product-card" key={product.id}>
                      <div className="store-product-image">
                        {product.imageUrl ? (
                          <img
                            ...
                            style={{ objectPosition: product.imageFocus?.main ?? '50% 50%' }}
                          />
                        ) : (
                          <AppIcon name="products" size={36} />
                        )}
                      </div>
                      <div className="store-product-copy">
                        ...
                        {product.variants.length > 0 && (
                          <small className="variant-hint">{t.options}</small>
                        )}
                      </div>
          ...
          <section className="store-checkout">
            <div className="checkout-main">
              <div className="mobile-checkout-header">
                <button aria-label={t.back} onClick={() => setCheckout(false)}>
                  <AppIcon name={language === 'ar' ? 'chevronRight' : 'chevronLeft'} />
                </button>
                <div>
                  <h2>{t.checkout}</h2>
                  <p>{t.details}</p>
              ...
              <aside className="checkout-summary">
                ...
                {bag.length === 0 && <p>{t.empty}</p>}
                {bag.map((line) => (
                  <div className="checkout-line" key={`${line.product.id}:${line.variant?.id}`}>
                    <div className="checkout-line-image">
                      {line.product.imageUrl ? (
                        <img
                          ...
                          alt=""
                          style={{
                            objectPosition: line.product.imageFocus?.main ?? '50% 50%',
                            objectFit: 'cover',
                          }}
                        />
                      ) : (
              ...
              <section className="checkout-step">
                ...
                <div className="choice-row checkout-choice">
                  ...
                  {store.allow_dine_in && (
                    <button
                      className={fulfillment === 'dine_in' ? 'selected' : ''}
                      onClick={() => chooseFulfillment('dine_in')}
                    >
                      <AppIcon name="dineIn" />
                      {t.dineIn}
                    </button>
              ...
              {fulfillment === 'delivery' && (
                <section className="checkout-step location-first">
                  <div className="checkout-step-title">
                    <span>3</span>
                    <div>
                      <h3>{t.where}</h3>
                  ...
                  <LocationPicker
                    ...
                    onChange={(latitude, longitude) =>
                      setCustomer((current) => ({ ...current, latitude, longitude }))
                    }
                  ...
                  />
                  <div
                    className="delivery-quote-strip"
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '8px',
                  ...
                  >
                    ...
                    {quote.status === 'quoted' && (
                      <>
                        ...
                        <span style={{ flex: 1 }}>
                          {t.deliveryFeeLabel}
                          {quote.providerName ? ` · ${quote.providerName}` : ''}
                        </span>
                  ...
                  <TextInput
                    ...
                    onChange={(event) =>
                      setCustomer({ ...customer, addressDetails: event.target.value })
                    }
                  />
                </section>
              )}
              {fulfillment === 'dine_in' && (
                <section className="checkout-step dine-in-table-step">
                  <div className="checkout-step-title">
                    <span>3</span>
                    <div>
                      <h3>{t.table}</h3>
                      <p>{tableLockedFromQr ? t.tableLocked : t.tableHelp}</p>
                    </div>
                  </div>
                  {tableLockedFromQr ? (
                    <div className="locked-table-card">
                    ...
                    <label className="table-select-field">
                      ...
                      >
                        {!tableNumber && <option value="">{t.chooseTable}</option>}
              ...
              <section className="checkout-step">
                <div className="checkout-step-title">
                  <span>{fulfillment === 'delivery' || fulfillment === 'dine_in' ? '4' : '3'}</span>
                  <div>
                    <h3>{t.payment}</h3>
                    <p>{paymentMethod === 'CASH' ? t.cashAtTableHelp : t.paymentHelp}</p>
                  </div>
                </div>
                <div className="choice-row checkout-choice">
                  {fulfillment === 'dine_in' && (
                  ...
                  {store.fawran_enabled &&
                    !(fulfillment === 'delivery' && quote.status === 'pending') && (
                      <button
                        className={paymentMethod === 'FAWRAN' ? 'selected' : ''}
                ...
                {fulfillment === 'delivery' && quote.status === 'pending' && (
                  <p style={{ fontSize: '0.82rem', color: '#92400e', margin: '8px 0 0' }}>
                    {t.deliveryPending}
                  </p>
                )}
                  ...
                  <div className="fawran-panel">
                    ...
                    <div className="fawran-details">
                      {store.fawran_mobile && (
                        <button
                          type="button"
                          className="fawran-copy-row"
                          onClick={() => copyToClipboard(store.fawran_mobile, t.fawranAlias)}
                        >
                      ...
                      {customer.name && (
                        <button
                          type="button"
                          className="fawran-copy-row"
                          onClick={() => copyToClipboard(customer.name, t.reference)}
                        >
                    ...
                    {!proof && (
                      <p style={{ fontSize: '0.8rem', color: '#b45309', margin: '6px 0 0' }}>
                        {t.proofRequired}
                      </p>
                    )}
                  </div>
        ...
        {confirmation && (
          <section className="order-confirmation">
            ...
            <p>{confirmation.order_number}</p>
            {confirmation.table_number && (
              <p>
                {t.table} {confirmation.table_number}
              </p>
            )}
            <strong>{money(Number(confirmation.total))}</strong>
            {confirmation.delivery_fee_status === 'PENDING' && (
              <p style={{ color: '#92400e', fontSize: '0.85rem' }}>{t.deliveryPendingConfirm}</p>
            )}
            <StatusText value={confirmation.payment_status} />
            <p className="confirmation-help">{t.shareThenTrack}</p>
            {confirmation.whatsapp_url && (
              <a
                className="whatsapp-share-button"
                href={confirmation.whatsapp_url}
                target="_blank"
            ...
            <p className="confirmation-countdown">
              {t.trackOrderHelp.replace('{{seconds}}', String(redirectSeconds))}
            </p>
          </section>
        )}
      </main>

      {bag.length > 0 && !checkout && !confirmation && (
        <button className="mobile-cart-dock" onClick={() => setCheckout(true)}>
          <span className="cart-dock-count">{itemCount}</span>
          <span>
            <b>{t.viewBag}</b>
            <small>{money(total)}</small>
          </span>
          <AppIcon name={language === 'ar' ? 'chevronLeft' : 'chevronRight'} />
        </button>
      )}

      {selecting && (
        <div className="public-dialog-backdrop" onClick={() => setSelecting(null)}>
          <div className="public-dialog" onClick={(event) => event.stopPropagation()}>
            ...
            >
              ...
              <div
                className="sheet-product-gallery"
                style={{
                  width: '100%',
                  display: 'flex',
                  flexDirection: 'column',
              ...
              >
                ...
                >
                  {productPhotos.length > 0 ? (
                    <img
                      src={productPhotos[activePhotoIdx]}
                      alt=""
                      style={{
                        width: '100%',
                        height: '100%',
                        objectFit: 'contain',
                        objectPosition: activeFocus,
                      }}
                    />
                  ) : (
                    <AppIcon name="products" size={64} />
                  )}
                </div>
              ...
              <div style={{ marginTop: '4px' }}>
                ...
                >
                  {language === 'ar' && selecting.nameAr ? selecting.nameAr : selecting.name}
                </h2>
                <strong
                  style={{ fontSize: '1.35rem', fontWeight: 800, color: 'var(--store-accent)' }}
                >
                  {selectedVariant ? money(selectedVariant.price) : money(selecting.price)}
                </strong>
                {selecting.description && (
                  ...
                  >
                    {language === 'ar' && selecting.descriptionAr
                      ? selecting.descriptionAr
                      : selecting.description}
                  </p>
                )}
              </div>
            </div>
            {selecting.variantGroups.map((group) => (
              <fieldset key={group.id}>
                ...
                <div className="choice-row">
                  {!group.required && (
                    <button
                      className={!selectedOptions[group.id] ? 'selected' : ''}
                      onClick={() =>
            ...
            >
              {t.continue}
              {selectedVariant ? ` · ${money(selectedVariant.price)}` : ''}
            </AppButton>
          </div>
        </div>
      )}
...
const StatusText: React.FC<{ value: string }> = ({ value }) => (
  <span className="status-pill warning">{value.replace(/_/g, ' ')}</span>
);
