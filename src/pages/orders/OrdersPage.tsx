...
export const OrdersPage: React.FC = () => {
  ...
  const nextStatus = (order: Order): OrderStatus | null => {
    ...
    if (order.status === 'PREPARING')
      return order.fulfillment === 'DELIVERY' ? 'OUT_FOR_DELIVERY' : 'COMPLETE';
  ...
  const columns: Column<Order>[] = [
    {
      ...
      header: 'Order ID',
      render: (row) => (
        <strong style={{ color: 'var(--dorzak-primary)', fontFamily: 'monospace' }}>
          {row.id}
        </strong>
      ),
    },
    {
      ...
      render: (row) => (
        <div>
          ...
          {row.fulfillment === 'DINE_IN' && row.tableNumber && (
            <div style={{ fontSize: '0.75rem', color: 'var(--dorzak-primary)', fontWeight: 800 }}>
    ...
    },
    {
      ...
      header: 'Total',
      render: (row) => (
        <strong style={{ fontSize: '1rem', color: 'var(--dorzak-success)' }}>
          {money(row.total)}
        </strong>
      ),
    },
    {
      ...
      render: (row) => {
        ...
        const canRefund =
          ...
          (row.paymentStatus === 'PAID' || row.paymentStatus === 'PENDING_VERIFICATION');
        return (
          <div style={{ display: 'flex', gap: '6px' }}>
            {canCancel && (
              <AppButton
                variant="secondary"
                style={{
                  borderColor: 'var(--dorzak-danger, #d32f2f)',
                  color: 'var(--dorzak-danger, #d32f2f)',
                }}
                onClick={async (e) => {
                  e.stopPropagation();
            ...
            {canRefund && (
              <AppButton
                ...
                onClick={async (e) => {
                  e.stopPropagation();
                  await updatePaymentStatus(row, 'REFUNDED');
                }}
              >
                Refund
              </AppButton>
            ...
            {activeTab === 'ACTIVE' &&
              row.paymentMethod === 'FAWRAN' &&
              row.paymentStatus === 'PENDING_VERIFICATION' && (
                <AppButton
                  variant="secondary"
                  onClick={async (e) => {
                    e.stopPropagation();
                    await updatePaymentStatus(row, 'PAID');
                  }}
                >
                  Verify Payment
                </AppButton>
            ...
            {canAdvance && (
              <AppButton
                ...
                onClick={async (e) => {
                  e.stopPropagation();
                  await updateStatus(row, nextStatus(row)!);
                }}
              >
            ...
            <AppButton
              ...
              onClick={(e) => {
                e.stopPropagation();
                openModal('ORDER_DETAIL', { order: row });
              }}
            >
      ...
      >
        <div>
          ...
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>
            Track active orders and browse fulfilled order history.
          </span>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
        ...
        >
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>
              {activeTab === 'ACTIVE' ? 'ACTIVE ORDER VALUE' : 'HISTORY REVENUE'}
            </div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--dorzak-success)' }}>
              {money(totalRevenue)}
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--dorzak-primary)' }}>
              {completedCount}
            </div>
          </div>
        </div>
            ...
            style={{
              ...
              backgroundColor:
                activeTab === 'ACTIVE'
                  ...
                  : 'var(--dorzak-danger-light, #fdecec)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              color:
                activeTab === 'ACTIVE' ? 'var(--color-warning)' : 'var(--dorzak-danger, #d32f2f)',
            }}
          >
            <AppIcon name={activeTab === 'ACTIVE' ? 'alert' : 'close'} size={20} />
          </div>
          <div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>
              {activeTab === 'ACTIVE' ? 'PENDING' : 'CANCELLED'}
            </div>
            <div
              style={{
                fontSize: '1.2rem',
                ...
                color:
                  activeTab === 'ACTIVE'
                    ? 'var(--dorzak-warning)'
                    : 'var(--dorzak-danger, #d32f2f)',
              }}
            >
              {activeTab === 'ACTIVE' ? pendingCount : cancelledCount}
            </div>
          </div>
        </div>
      </div>
          ...
          {(activeTab === 'ACTIVE'
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid var(--color-border)',
                backgroundColor:
                  statusFilter === s ? 'var(--dorzak-primary)' : 'var(--color-surface)',
