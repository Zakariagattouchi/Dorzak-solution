...
export const PlatformShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--color-bg)' }}>
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px', background: '#17201e', color: '#fff' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: 'var(--dorzak-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800 }}>D</div>
          <div>
            <div style={{ fontWeight: 700, fontSize: '0.95rem', lineHeight: 1.1 }}>Dorzak Platform</div>
            <div style={{ fontSize: '0.72rem', opacity: 0.7 }}>Super admin console</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <span style={{ fontSize: '0.82rem', opacity: 0.85 }}>{user?.name} · {user?.email}</span>
          <button onClick={handleLogout} style={{ background: 'rgba(255,255,255,0.12)', color: '#fff', border: 'none', borderRadius: '6px', padding: '6px 12px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' }}>
            Sign out
          </button>
        </div>
      </header>

      <main style={{ flex: 1, padding: '24px', maxWidth: '1120px', width: '100%', margin: '0 auto' }}>
        {children}
      </main>
      <ToastHost />
    </div>
