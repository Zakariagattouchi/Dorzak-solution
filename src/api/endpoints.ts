...
const PUBLIC_BASE = ((import.meta as any).env?.VITE_API_URL || '/api/v1').replace(
  /\/api\/v1$/,
  '/api/public',
);
...
export const settingsApi = {
  ...
  update: (group: string, payload: Record<string, unknown>) =>
    request(`/settings/${group}`, { method: 'PUT', body: payload }),
...
export const loyaltyApi = {
  ...
  update: (payload: Record<string, unknown>) =>
    request('/settings/loyalty', { method: 'PUT', body: payload }),
};
...
export const messagingApi = {
  ...
  update: (payload: Record<string, unknown>) =>
    request('/settings/messaging', { method: 'PUT', body: payload }),
...
export const couponsApi = {
  ...
  create: (payload: Record<string, unknown>) =>
    request('/coupons', { method: 'POST', body: payload }),
  update: (id: number, payload: Record<string, unknown>) =>
    request(`/coupons/${id}`, { method: 'PATCH', body: payload }),
...
export const referralApi = {
  ...
  update: (payload: Record<string, unknown>) =>
    request('/settings/referrals', { method: 'PUT', body: payload }),
};
...
export const giftCardsApi = {
  ...
  redeem: (code: string, customerId: number) =>
    request('/gift-cards/redeem', { method: 'POST', body: { code, customer_id: customerId } }),
};

export const segmentsApi = {
  list: () => request('/segments'),
  create: (payload: Record<string, unknown>) =>
    request('/segments', { method: 'POST', body: payload }),
...
export const staffApi = {
  ...
  update: (userId: number, payload: { role?: string; is_active?: boolean }) =>
    request(`/staff/${userId}`, { method: 'PATCH', body: payload }),
  remove: (userId: number) => request(`/staff/${userId}`, { method: 'DELETE' }),
  acceptInvitation: (
    token: string,
    payload: { password: string; password_confirmation: string; name?: string },
  ) => request(`/staff/invitations/${token}/accept`, { method: 'POST', body: payload }),
};

export const catalogApi = {
  categories: () => request('/categories'),
  createCategory: (payload: { name: string; color?: string; description?: string }) =>
  ...
  reorderCategories: (ids: number[]) =>
    request('/categories/reorder', { method: 'PATCH', body: { ids } }),
...
export const customerApi = {
  ...
  create: (payload: Record<string, unknown>) =>
    request('/customers', { method: 'POST', body: payload }),
  update: (id: number, payload: Record<string, unknown>) =>
    request(`/customers/${id}`, { method: 'PUT', body: payload }),
...
export const orderApi = {
  ...
  setDeliveryFee: (id: number, fee: number) =>
    request(`/orders/${id}/delivery-fee`, { method: 'PATCH', body: { delivery_fee: fee } }),
...
export const reportApi = {
  finance: (period = 'all', params?: Record<string, string>) =>
    request('/reports/finance', { params: { period, ...params } }),
...
export const subscriptionApi = {
  ...
  plans: () => request('/plans'),
  startTrial: (planId: number) =>
    request('/subscription/trial', { method: 'POST', body: { plan_id: planId } }),
...
export const platformApi = {
  ...
  auditLogs: (params?: Record<string, string>) =>
    request('/platform/audit-logs', { base: PLATFORM_BASE, params }),
  ...
  deliveryProviders: {
    ...
    create: (payload: Record<string, unknown>) =>
      request('/platform/delivery-providers', {
        base: PLATFORM_BASE,
        method: 'POST',
        body: payload,
      }),
    update: (id: number, payload: Record<string, unknown>) =>
      request(`/platform/delivery-providers/${id}`, {
        base: PLATFORM_BASE,
        method: 'PUT',
        body: payload,
      }),
    destroy: (id: number) =>
      request(`/platform/delivery-providers/${id}`, { base: PLATFORM_BASE, method: 'DELETE' }),
  },
  plans: {
    list: () => request('/platform/plans', { base: PLATFORM_BASE }),
    create: (payload: Record<string, unknown>) =>
      request('/platform/plans', { base: PLATFORM_BASE, method: 'POST', body: payload }),
    update: (id: number, payload: Record<string, unknown>) =>
      request(`/platform/plans/${id}`, { base: PLATFORM_BASE, method: 'PUT', body: payload }),
    destroy: (id: number) =>
      request(`/platform/plans/${id}`, { base: PLATFORM_BASE, method: 'DELETE' }),
  },
  stores: {
    list: (params?: Record<string, string>) =>
      request('/platform/stores', { base: PLATFORM_BASE, params }),
    ...
    suspend: (id: number) =>
      request(`/platform/stores/${id}/suspend`, { base: PLATFORM_BASE, method: 'PUT' }),
    reactivate: (id: number) =>
      request(`/platform/stores/${id}/reactivate`, { base: PLATFORM_BASE, method: 'PUT' }),
    assignPlan: (id: number, planId: number) =>
      request(`/platform/stores/${id}/plan`, {
        base: PLATFORM_BASE,
        method: 'PUT',
        body: { plan_id: planId },
      }),
    impersonate: (id: number) =>
      request(`/platform/stores/${id}/impersonate`, { base: PLATFORM_BASE, method: 'POST' }),
    destroy: (id: number, confirmName: string) =>
      request(`/platform/stores/${id}`, {
        base: PLATFORM_BASE,
        method: 'DELETE',
  ...
  customers: (params?: Record<string, string>) =>
    request('/platform/customers', { base: PLATFORM_BASE, params }),
  products: (params?: Record<string, string>) =>
    request('/platform/products', { base: PLATFORM_BASE, params }),
  exportCsv: (kind: 'customers' | 'products' | 'stores' | 'orders') =>
    downloadFile(
      `/platform/${kind}/export`,
  ...
  users: {
    list: (params?: Record<string, string>) =>
      request('/platform/users', { base: PLATFORM_BASE, params }),
    grantAdmin: (id: number) =>
      request(`/platform/users/${id}/grant-admin`, { base: PLATFORM_BASE, method: 'PUT' }),
    revokeAdmin: (id: number) =>
      request(`/platform/users/${id}/revoke-admin`, { base: PLATFORM_BASE, method: 'PUT' }),
    setActive: (id: number, isActive: boolean) =>
      request(`/platform/users/${id}/active`, {
        base: PLATFORM_BASE,
        method: 'PUT',
        body: { is_active: isActive },
      }),
    resetPassword: (id: number) =>
      request(`/platform/users/${id}/reset-password`, { base: PLATFORM_BASE, method: 'POST' }),
  },
};
...
export const publicApi = {
  ...
  deliveryQuote: (
    slug: string,
    params: { lat: string; lng: string; subtotal: string; delivery_provider_id?: string },
  ) => request(`/stores/${slug}/delivery-quote`, { base: PUBLIC_BASE, params, auth: false }),
  catalog: (slug: string, params?: Record<string, string>) =>
    request(`/stores/${slug}/catalog`, { base: PUBLIC_BASE, params, auth: false }),
  createOrder: (slug: string, payload: Record<string, unknown> | FormData) =>
    request(`/stores/${slug}/orders`, {
      base: PUBLIC_BASE,
      method: 'POST',
      body: payload,
      auth: false,
  ...
  uploadOrderPaymentProof: (slug: string, orderNumber: string, file: File) => {
    ...
    return request(`/stores/${slug}/orders/${orderNumber}/payment-proof`, {
      base: PUBLIC_BASE,
      method: 'POST',
      body: fd,
      auth: false,
  ...
  lookupCustomer: (slug: string, phone: string) =>
    request(`/stores/${slug}/customers/lookup`, {
      base: PUBLIC_BASE,
      params: { phone },
      auth: false,
    }),
