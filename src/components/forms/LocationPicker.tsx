...
const MapEvents: React.FC<LocationPickerProps> = ({ latitude, longitude, onChange }) => {
  ...
  React.useEffect(() => {
    if (latitude != null && longitude != null) map.flyTo([latitude, longitude], Math.max(map.getZoom(), 14));
  }, [latitude, longitude, map]);
  return latitude != null && longitude != null
    ? <CircleMarker center={[latitude, longitude]} radius={9} pathOptions={{ color: '#0f766e', fillColor: '#14b8a6', fillOpacity: 1 }} />
    : null;
};

export const LocationPicker: React.FC<LocationPickerProps> = ({ latitude, longitude, onChange, onAddressResolved }) => {
  const [query, setQuery] = useState('');
  const [searching, setSearching] = useState(false);
  const center: [number, number] = latitude != null && longitude != null ? [latitude, longitude] : [25.2854, 51.531];
  ...
  return (
    <div className="location-picker">
      <div className="location-tools">
        <TextInput aria-label="Search location" placeholder="Search address or area" value={query} onChange={(event) => setQuery(event.target.value)} />
        <AppButton type="button" variant="secondary" loading={searching} onClick={search}>Search</AppButton>
        <AppButton type="button" variant="secondary" icon="mapPin" onClick={locate}>Current Location</AppButton>
      </div>
