...
export const OrderDetailModal: React.FC = () => {
  ...
  const paymentIcon: Record<string, IconName> = {
    CARD: 'card',
    CASH: 'cash',
    TRANSFER: 'transfer',
    WHATSAPP: 'whatsapp',
    ...
    <BaseModal
      ...
      footer={
        <>
          ...
          {order.status === 'CANCELLED' &&
            (order.paymentStatus === 'PAID' || order.paymentStatus === 'PENDING_VERIFICATION') && (
              <AppButton
                variant="secondary"
                style={{
                  borderColor: 'var(--dorzak-danger, #d32f2f)',
                ...
                onClick={async () => {
                  await updatePaymentStatus(order, 'REFUNDED');
                }}
              >
                Refund
              </AppButton>
          ...
          {!isHistory &&
            ...
            order.paymentStatus === 'PENDING_VERIFICATION' && (
              <AppButton variant="primary" onClick={handleVerifyPayment}>
                Verify Payment
              </AppButton>
            )}
          ...
          {order.hasPaymentProof && order.apiId && (
            <AppButton variant="secondary" onClick={handleViewProof}>
              View Proof
            </AppButton>
          )}
          <AppButton variant="secondary" onClick={handlePrintReceipt} icon="orders">
            Receipt
          </AppButton>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        {feePending && feeEditable && (
          ...
          >
            ...
            <div style={{ fontSize: '0.85rem', color: '#92400e' }}>
              Check the drop-off location, then set the delivery fee — the customer will see the
              updated total and pay from their order page.
            </div>
        ...
        >
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--dorzak-success)' }}>
        ...
        >
          ...
          <div className="card" style={{ padding: '14px' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: '1rem', textTransform: 'capitalize' }}>
              {(order.fulfillment ?? 'PICKUP').replace(/_/g, ' ').toLowerCase()}
            </div>
            ...
            {order.fulfillment === 'DELIVERY' && (
              ...
              >
                {order.pickupAddress && (
                  <span>
                    <strong>Collect from:</strong> {order.pickupAddress}
                  </span>
                )}
                <span>
              ...
              {order.items.map((item: OrderItem, idx: number) => (
                <tr key={idx} style={{ borderBottom: '1px solid var(--color-border)' }}>
                  ...
                  <td style={{ padding: '10px 14px', textAlign: 'right' }}>
                    {money(item.unitPrice, 2, order.currencyCode)}
                  </td>
          ...
          {order.fulfillment === 'DELIVERY' && (
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ color: 'var(--text-muted)' }}>
                Delivery{order.deliveryProviderName ? ` · ${order.deliveryProviderName}` : ''}
                {order.deliveryDistanceKm != null ? ` (${order.deliveryDistanceKm} km)` : ''}
              </span>
